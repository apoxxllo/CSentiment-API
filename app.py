...
import nltk
from flask import Flask, request, json, jsonify, make_response, render_template, send_from_directory
...
cors = CORS(app)
DEMO_STATIC_FOLDER = os.path.join('..', 'Demo', 'static')

@app.route('/demo_static/<path:filename>')
def demo_static(filename):
    return send_from_directory(DEMO_STATIC_FOLDER, filename)
...
def generateReport():
    ...
    options = {'page-size': 'legal', 'encoding': 'utf-8', 'margin-top': '0.5cm', 'margin-bottom': '0cm',
               'margin-left': '0.5cm', 'margin-right': '0.5cm', "enable-local-file-access": ""}
